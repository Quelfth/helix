...
pub(crate) fn compute_semantic_tokens_for_view(
    ...
    let language_server = doc.language_servers().next()?;
    ...
    let first_visible_line = doc_text.char_to_line(doc.view_offset(view.id).anchor);
    ...
    let callback = super::super::make_job_callback(
        ...
        move |editor, _compositor, response: Option<lsp::SemanticTokensRangeResult>| {
            ...
            {
                let Some((ls, data)) = (match (doc.language_servers().next(), response) {
                    (
                        Some(ls),
                        Some(
                ...
                for token in data {
                    ...
                    let mut vec = (*dst.tokens).to_owned();

                    vec.push((range, tokens_for_range));
                    dst.tokens = vec.into();
                }
            }
